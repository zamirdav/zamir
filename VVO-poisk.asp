...
<%
'07_09_2023  начал программу ОСК для ввода информации из регионов
' использовал наработки из выселенцев
...
'
Tim_start=Time
'Response.Write "Довлетбаев Замир Самарбекович <BR>"<!--©(с)-->
...
if T="1" then
   if F1&"~" = "~" then
      Response.Write "не заполнена фамилия, чтобы вернуться нажми слева вверху кнопку 'назад'(<-) <BR>"   
'      Response.Write "<A href='vvo-form.asp?T=1'>не заполнена фамилия, вернуться?</A>"  
   else
      if F2&"~" = "~" then
         Response.Write "не заполнено имя, чтобы вернуться нажми слева вверху кнопку 'назад'(<-) <BR>"   
'      Response.Write "<A href='vvo-form.asp?T=1'>не заполнено имя, вернуться?</A>"  
      else
         if DR&"~" = "~" then
           Response.Write "нет даты рождения, чтобы вернуться нажми слева вверху кнопку 'назад'(<-) <BR>"   
'            Response.Write "<A href='vvo-form.asp?T=1'>не заполнена дата рождения, вернуться?</A>"  
'            Break
         else
           Call POISK_ZAD_V("SP", 1, "F1", "F2", "F3", "DR", "поселенцы")     
           Response.Write "<HR><p><font size='5' > "  
'          if KOL_ZAP=0 then
	   Response.Write "<A href='add_iF.asp?F1=" & F1 & "&F2=" & F2 & "&F3=" & F3  & "&DR=" & DR& "'>все таки новая запись?</A>"
'	   end if
...
%>
<A href="VVO-form.asp?T=1"><font size =+1 >Выход без записи?</FONT></A><BR>
