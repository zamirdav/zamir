...
sub sp_bok(bok,file_out)  ' готовит одну таблицу из строк(в данном случаи 12 месяцев) для общего отчета из бок
   ...
   ZAPR = "CREATE TABLE "& file_out&"(bok int);"  'создаем табл
...
'm=p+1  ' p-месяцы+1 меньше которого должен быть
...
sub sp_count_plus(file0,file1,file2,file3,file4,file5,file6,file7,file8,file9,file10,file11,file12,file_out)  ' боковины шапки отчетов с услвием
   ZAPR = "DROP TABLE IF EXISTS "&file_out&";"       'если есть табл с таким именем удаляем
   On Error Resume Next
   Set rStat = db.Execute(ZAPR)  
   if db.Errors.Count > 0 then
...
call sp_bok(bok,"d_bok")'создает таблицу согласно из р строк

'программа в цикле создает таблицы согласно 
p=1
do WHIlE P<13
   poisk=" where YEAR(AT_V_D)=2023 AND MONTH(AT_V_D)="&P   ' можно вставлять поиск допустим "where r5=1"
